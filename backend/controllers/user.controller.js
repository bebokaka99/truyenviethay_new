...
exports.getAllUsers = async (req, res) => {
    const { search, page = 1, limit = 10, role, sortBy, sortOrder } = req.query; 
    const offset = (page - 1) * limit;

    try {
        // Lấy danh sách người dùng và tổng số cho phân trang
        const { users, total } = await User.findAllUsers(search, parseInt(limit), offset, role, sortBy, sortOrder);
